"""
Script created from gist - https://gist.github.com/Lonami/b09fc1abb471fd0b8b5483d54f737ea0 and modified to return
refused status
"""
import struct
import socket
...
class Server:
    def __init__(self, data=None):
        if not data:
            ...
            self.players = Players(data.get("players", None))
            if "version" in data:
    ...
    def __str__(self):
        return 'Server(description={!r}, icon={!r}, version={!r}, '\
                'protocol={!r}, players={}, accepting_players={})'.format(
            self.description, bool(self.icon), self.version,
            self.protocol, self.players, self.accepting_players
        )
