...
class MappedQueue(object):
    ...
    def binary_search(self, value):
        """
        Performs a binary search on the underlying heap and returns the index corresponding to the closest value.
        Access this value via mapped_queue.h
        """
        ...
        while lo <= hi:
            ...
            if self.h[mid] < value:
                lo = mid + 1
            elif self.h[mid] > value:
                hi = mid - 1
            else:
                best_ind = mid
                break
            # check if data[mid] is closer to val than data[best_ind]
            if abs(self.h[mid] - value) < abs(self.h[best_ind] - value):
                best_ind = mid
        return best_ind